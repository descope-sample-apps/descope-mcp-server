...
export const createServer = () => {
  ...
  server.tool(
    ...
    async ({
      ...
      try {
        ...
        const limitedAudits = auditResponse
          ? auditResponse.slice(0, limit)
          : [];
  ...
  server.tool(
    ...
    {
      // Search parameters
      text: z.string().optional().describe("Text to search for in user fields"),
      emails: z
        .array(z.string())
        .optional()
      ...
      statuses: z
        .array(z.enum(["enabled", "disabled", "invited"]))
        .optional()
        .describe(
      ...
      testUsersOnly: z.boolean().optional().describe("Return only test users"),
      page: z.number().min(0).optional().describe("Page number for pagination"),
      limit: z
        .number()
        .min(1)
        .max(100)
  ...
  server.tool(
    ...
    {
      loginId: z.string().describe("Primary login identifier for the user"),
      additionalLoginIds: z
        .array(z.string())
        .optional()
      ...
      givenName: z.string().optional().describe("User's given/first name"),
      middleName: z.string().optional().describe("User's middle name"),
      familyName: z.string().optional().describe("User's family/last name"),
      picture: z
        .string()
        .url()
        .optional()
      ...
      templateOptions: z
        .object({
          ...
          customClaims: z
            ...
            .optional()
            .describe(
              "Custom claims to include in the template (as JSON string)"
            ),
        })
        .optional()
      ...
      try {
        ...
        if (templateOptions) {
          ...
          if (templateOptions.customClaims) {
            inviteOptions.templateOptions.customClaims =
              templateOptions.customClaims;
          }
        }

        const user = await descope.management.user.invite(
          loginId,
          inviteOptions
        );
  ...
  // Add update-user tool
  server.tool(
    "update-user",
    "Update a user in Descope project",
    {
      loginId: z
        .string()
        .describe("Primary login identifier for the user to update"),
      email: z.string().email().optional().describe("User's new email address"),
      displayName: z.string().optional().describe("User's new display name"),
      givenName: z.string().optional().describe("User's new given/first name"),
      middleName: z.string().optional().describe("User's new middle name"),
      familyName: z.string().optional().describe("User's new family/last name"),
      picture: z
        .string()
        .url()
        .optional()
        .describe("URL to user's new profile picture"),
      roles: z
        .array(z.string())
        .optional()
        .describe("New global role names to assign to the user"),
      customAttributes: z
        .record(z.any())
        .optional()
        .describe("New custom attributes for the user"),
    },
    async ({ loginId, ...options }) => {
      try {
        const user = await descope.management.user.update(loginId, options);
  ...
  // Add user-add-tenants tool
  server.tool(
    "user-add-tenants",
    "Add user to tenants in Descope project",
    {
      loginId: z.string().describe("Primary login identifier for the user"),
      tenantIds: z.array(z.string()).describe("Tenant IDs to add the user to"),
    },
    async ({ loginId, tenantIds }) => {
      try {
        for (const tenantId of tenantIds) {
          await descope.management.user.addTenant(loginId, tenantId);
        }
        ...
        return {
          content: [
            {
              type: "text",
              text: `Successfully added user with loginId ${loginId} to tenants: ${tenantIds.join(
                ", "
              )}`,
            },
          ],
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error adding user to tenants: ${error}`,
            },
          ],
        };
      }
  ...
  // Add create-audit-event tool
  server.tool(
    "create-audit-event",
    "Create a custom audit event in Descope project",
    {
      action: z.string().describe("The action that was performed"),
      type: z
        .enum(["info", "warn", "error"])
        .describe("The type of event (info, warn, or error)"),
      actorId: z
        .string()
        .describe("The ID of the user who performed the action"),
      tenantId: z
        .string()
        .describe("The ID of the tenant where the action was performed"),
      data: z
        .record(z.any())
        .optional()
        .describe("Any additional data to include with the event"),
    },
    async (options) => {
      try {
        await descope.management.audit.createEvent({
        ...
        return {
          content: [
            {
              type: "text",
              text: "Successfully created audit event.",
            },
          ],
        };
      } catch (error) {
        return {
          content: [
            {
              type: "text",
              text: `Error creating audit event: ${error}`,
            },
          ],
        };
      }
  ...
  // Add delete-user tool
  server.tool(
    "delete-user",
    "Delete a user in Descope project",
    {
      loginId: z
        .string()
        .describe("Primary login identifier for the user to delete"),
    },
    async ({ loginId }) => {
      try {
        await descope.management.user.delete(loginId);

        return {
          content: [
            {
              type: "text",
              text: `Successfully deleted user: ${loginId}`,
            },
          ],
        };
      } catch (error) {
  ...
  // Add search-roles tool
  server.tool(
    "search-roles",
    "Search for roles in Descope project",
    {},
    async () => {
      try {
        const roles = await descope.management.role.search({});
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error searching roles: ${error}`,
            },
          ],
        };
      }
  ...
  // Add list-all-users tool
  server.tool(
    "list-all-users",
    "List all users in Descope project (up to 100)",
    {},
    async () => {
      try {
        const users = await descope.management.user.search({ limit: 100 });
  ...
  // Add search-tenants tool
  server.tool(
    "search-tenants",
    "Search for tenants in Descope project",
    {},
    async () => {
      try {
        const tenants = await descope.management.tenant.searchAll([]);
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error searching tenants: ${error}`,
            },
          ],
        };
      }
